...
void computeSequenceSimilarity(vector<Graph> &graph_dataset, unordered_map<unsigned, unordered_set<unsigned> > &candidate_pairs, vector<int> &global_score_freq, double simScore_threshold, unsigned long &simPairCount, vector<pair<pair<unsigned, unsigned>, double> > &g_res);
...
int main(int argc, char const *argv[])
{
	...
	unsigned long loose_filter_count=0, strong_filter_count=0, banding_filter_count=0, final_cand_pair_count=0, simPairCount=0;
	vector<pair<pair<unsigned, unsigned>, double> > g_res;

	// timestamping start time
	// Preprocesses the pruned graph pairs 
	preProcess(graph_dataset, choice); 
	...
	applyFilters(graph_dataset, candidate_graphs, candidate_pairs, dataset_size, choice, simScore_threshold, loose_filter_count, strong_filter_count);
	...
	if(choice == 3)
	{
		...
		computeSequenceSimilarity(graph_dataset, final_candidate_pairs, global_score_freq, simScore_threshold, simPairCount, g_res);
	}	
	else
	{
		// Computing naive sequence similarity
		computeSequenceSimilarity(graph_dataset, candidate_pairs, global_score_freq, simScore_threshold, simPairCount, g_res);
	}
	...
	stat_file << "Total Time Taken: "<< (clocksTosec(cl0,cl1) + clocksTosec(cl2,cl3))  << " milliseconds" << endl;
	stat_file.close();

	cout << "Memory used: " << memoryUsage() << " MB" << endl; // for displaying total memory usage (in MB)
	cout << "Total Time Taken: "<< (clocksTosec(cl0,cl1) + clocksTosec(cl2,cl3)) << " milliseconds" << endl;
	...
	{
		all_graph_file << g1->first.first << " " << g1->first.second << " " << g1->second << endl;
	}
...
// final sequence similarity computation of pruned pairs
void computeSequenceSimilarity(vector<Graph> &graph_dataset, unordered_map<unsigned, unordered_set<unsigned> > &final_cand_pairs, vector<int> &global_score_freq, double simScore_threshold, unsigned long &simPairCount, vector<pair<pair<unsigned, unsigned>, double> > &g_res)
{
